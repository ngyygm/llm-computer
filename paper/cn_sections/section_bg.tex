...
图~\ref{fig:classical_stack}展示了六层经典计算栈及其与ICA层的对应关系。
以下逐一介绍各层的核心概念。

\input{figures/fig_classical_stack}
...
解耦式服务架构将这种预填充/解码的解耦推到集群规模：Splitwise\cite{patel2024splitwise}和Mooncake\cite{qin2024mooncake}将两个阶段分别部署在专用的GPU池上，并把KV缓存视为一种全局的、可迁移的资源，在超大规模上实现了"KV缓存作为硬件管理缓存"的类比。
...
Codex\cite{openai_codex_overview_2026}和Claude Code\cite{anthropic_claudecode_overview_2026}是工业级的代码Agent，具备子Agent调度\cite{anthropic_claudecode_subagents_2026}、沙箱隔离\cite{openai_codex_sandbox_2026,anthropic_claudecode_sandbox_2026}和权限控制\cite{openai_codex_approvals_2026,anthropic_claudecode_security_2026}等类似操作系统内核的能力。
...
\input{/home/linkco/exa/llm-computer/paper/figures/fig_dual_stack.tex}
