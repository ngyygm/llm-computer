...
在全文中，我们将每个挑战追溯到ICA层次（第~\ref{sec:icam}节）和设计启发式（第~\ref{sec:laws}节），展示第~\ref{sec:framework}节引入的双平面架构如何作为这些挑战的\emph{统一组织视角}。
...
启发式I（第~\ref{sec:law1}节）刻画了这一权衡的缓存侧：KV缓存命中率$H$直接决定了可达到的加速比，任何调度或内存管理优化都可以理解为在三元约束下最大化$H$的尝试。
...
启发式II（第~\ref{sec:law2}节）当前的模型尚未捕捉这一时序维度，这是后续研究的重要方向。
