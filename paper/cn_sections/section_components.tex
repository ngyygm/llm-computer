...
两者之所以相似，是因为它们在底层与行为之间共享同一种关系。
在CPU与基础模型中，底层都是固定的，而行为通过外部层来定制——CPU之上是程序，模型之上是提示词与工具schema——于是同一块硅片既能运行文字处理器，也能运行科学模拟，正如同一组权重既能写代码、分析数据，也能翻译语言、规划任务。
两者也都保持着接口稳定，而实现则在接口之下持续演进。
CPU通过ISA（算术、跳转、访存）向上层暴露操作，模型通过提示模板和工具schema（推理、生成、工具调用）向上层暴露操作，并都遵循``接口稳定、实现可演进''的原则——x86架构存在了40多年，而底层微架构从单标量演进到了超标量乱序执行，GPT-4的API接口保持稳定，而底层模型版本持续更新。
在这一接口之下，是一片丰富的优化空间，工程师在两者中都不打破上层契约地追逐性能：CPU有流水线、分支预测、超标量发射等微架构优化，模型则有FlashAttention（IO感知的注意力计算\cite{dao2022flashattention}）、推测解码（用小模型预测大模型输出\cite{leviathan2023speculative}）和连续批处理等优化，同样面对``接口不变、底层性能持续提升''的工程挑战。
...
KV缓存与CPU缓存的相似性体现在工程机制的层面，其中有四项机制几乎一一对应。

\paragraph{对时间局部性的共同依赖。}
...
上下文管理与虚拟内存的相似性体现在\textbf{机制层面}，并沿着三个维度彼此呼应。
...
Agent运行时与操作系统的相似性是所有类比中最为深刻的，因为它体现在\textbf{架构职责}的层面：两者都承担起同一组三项职责。
