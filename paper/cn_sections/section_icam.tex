...
\item \textbf{L5 编排层}：任务规划、Agent调度、权限审批、失败恢复、状态提交。这一层相当于经典体系结构中的操作系统内核——它负责任务的调度、资源分配、安全隔离和容错。OpenAI Codex将Agent任务提交到沙箱中执行，支持子Agent的创建与协调\cite{openai_codex_overview_2026}；Claude Code通过skill机制扩展Agent能力，通过MCP连接外部工具\cite{anthropic_claudecode_overview_2026,anthropic_claudecode_skills_2026}——它们是L5的代表实现。
...
ICA的分层是一种刻意的、务实的映射，而非唯一强制的拆分：六个层次各自与一个经典层级一一对应（表~\ref{tab:icam}），并以与其经典对应物不同的"核心差异"为特征，每个边界都可通过第~\ref{sec:icam-interfaces}节的接口契约独立检验。其组织原则与网络协议栈或操作系统层次结构相同——每一层为上层提供服务，同时只依赖下层的接口——这种分层的好处是\textbf{关注点分离}：L2的工程师优化KV缓存策略时无需关心Agent的编排逻辑，L5的开发者设计任务调度时无需理解注意力计算的硬件实现。我们将此分解作为一种组织性提案；随着领域成熟，具体的边界几乎必然会被进一步细化（第~\ref{sec:conclusion}节）。
...
ICA同样为每对相邻层定义接口契约。图~\ref{fig:interface_contracts}展示了全部五个接口契约的总览。下文逐一说明每个接口的操作、不变量和度量指标。
...
\item \textbf{度量}：KV命中率（$H$，即启发式I的核心参数）、显存峰值、页置换次数。
\end{itemize}
...
\item \textbf{不变量}：每个返回的上下文条目都携带单调递增的版本戳和检索时间戳，类似于文件系统保证读取返回最新已提交版本（测试：在该条目的TTL内重新发起同一读取，返回的版本戳保持一致）。
...
\item \textbf{不变量}：任务执行产生可追踪trace；副作用操作是可补偿的：每个副作用操作都配对一个已记录的补偿动作，使得失败时触发补偿重放（测试：任务失败后，trace中不存在未补偿的副作用操作）——类似于数据库的事务原子性保证。
...
L4 & 工具结果的语义理解 & 工具schema、协议规范、capability标注、审计 \\
...
第二，\textbf{确定性平面的重心偏上}，主要覆盖L5（编排层）和L4的接口契约部分（审计、capability标注），并延伸到L3的状态管理部分（记忆保留策略、版本戳）。这是因为系统级的安全、可靠、可审计需求需要确定性保证。

L4是一个特殊的"过渡层"：工具schema和协议规范属于确定性平面（它们是形式化的接口定义），而工具结果的语义理解属于概率平面（LLM需要理解自然语言描述的返回值）。
L6（应用层）是两个平面的消费者：用户同时获得概率平面的智能输出和确定性平面的可审计轨迹。因此，平面边界应理解为以L3-L4为中心的渐变接口，而非单层硬切分；表~\ref{tab:dual-plane-icam}的$6\times 2$矩阵是权威映射。
...
本文将这些原则迁移到模型原生计算领域，结合模型原生计算的特殊性，提出六条设计公理。每条公理指明其经典来源、在ICA中的体现以及具体的设计含义。图~\ref{fig:axioms_overview}总结了每条公理在ICA六个层次上的适用范围。
...
\item[设计含义] ICA的每一层都应基于局部性做缓存/预取。启发式I（第~\ref{sec:laws}节）将这一公理刻画为一个可估算的公式。
\end{description}
