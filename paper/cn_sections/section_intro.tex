...
其二，系统梳理相关研究工作，揭示现有隐喻之间的冲突，提出\textbf{双平面架构}作为统一解答---将模型原生系统栈划分为"概率执行平面"（模型推理与生成）和"确定性控制平面"（系统调度与控制），两平面通过明确定义的接口交互。

其三，在此基础上提出\textbf{智能计算体系结构（ICA）}---一个六层分层框架，包含层间接口契约与六条设计公理。
