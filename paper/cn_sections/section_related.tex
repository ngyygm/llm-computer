...
为确保覆盖的系统性，我们在arXiv、DBLP、IEEE Xplore、ACM Digital Library和Semantic Scholar五个来源上进行了检索，时间窗口为2022年1月至2026年5月，并从种子综述（例如Mi等人\cite{mi2025llmagentscomputersystems}）出发进行向后与向前的引用滚雪球扩展。检索查询词由一个架构/系统词（\emph{操作系统}、\emph{计算机体系结构}、\emph{虚拟内存}、\emph{指令集}、\emph{缓存}）与一个LLM词（\emph{LLM}、\emph{Agent}、\emph{KV cache}、\emph{推理服务}、\emph{工具调用}、\emph{多Agent}）组合而成。从约450条去重后的记录中，我们保留了188条满足以下条件之一的工作：(i)~为模型原生计算引入了系统级的抽象或机制；或(ii)~提供了直接支撑某条设计启发式的实证证据。我们按功能主题而非时间顺序组织保留文献集，并承认一个年轻且快速演进的领域使任何快照都具有临时性；附带的覆盖图（图~\ref{fig_coverage_map}）则明确展示了其范围与空白。
...
换言之，他们将LLM不是一个"聊天机器人"，而是一个"内核进程"——这一比喻后来被Karpathy在社交媒体上进一步传播。
在本文的类比体系中，Ge等人的工作直接对应"Agent OS（ICA编排层）"的概念萌芽。
...
本文在第\ref{sec:icam}节提出的ICA模型将这些不同的"kernel"概念统一到一个6层体系结构中，并明确LLM应被定位为\textbf{概率执行平面}的核心（侧重于物理执行层与推理服务层，即L1--L2，并渗透到上下文管理层L3），而Agent OS是\textbf{确定性控制平面}中的编排层——两个平面在L3--L4附近形成\textbf{渐变交叉/梯度过渡}，而非硬切分。
...
2026年初，Codex CLI引入了subagent（子Agent）功能——主Agent可以生成专门的子Agent并行执行子任务，每个子Agent拥有独立的上下文窗口和沙箱。
...
Agent运行时如今已能在庞大的代码库上支撑多小时自主执行\cite{anthropic2026agenticreport}（见第\ref{sec:agentevolution}节），这意味着它们必须处理远超当前交互式会话的状态持久化、检查点恢复和跨步骤资源管理问题。

OpenHands\cite{openhands_paper_2025}（前身OpenDevin）是一个开源的通用Agent平台，发表于ICLR 2025。
据报告，它在SWE-bench Verified基准上达到约53\%的问题解决率\cite{openhands_paper_2025,swebench2023}，支持多种LLM后端。
...
本文的ICA模型为这些框架提供了统一的理论框架，将它们的功能映射到明确的体系结构层次中。
...
在本文的ICA模型中，这分别对应第4层（I/O与工具层）和第5层（互联与协议层）。
...
本文的ICA模型将多Agent协作映射到第5层（互联与协议层）和第6层（应用层），并讨论了跨层交互的设计原则。
...
图~\ref{fig_coverage_map}给出了现有工作对各系统层次的覆盖情况。
...
这三种定位导致了完全不同的系统设计。
本文提出的ICA模型给出统一答案：LLM是概率执行平面的核心（类比CPU，侧重于L1--L2并渗透到L3），Agent OS是确定性控制平面的编排层（类比OS kernel），两个平面在L3--L4附近形成渐变交叉/梯度过渡，而非硬切分。
LLM提供计算能力，Agent OS提供管理与调度——正如CPU与OS kernel在传统体系结构中是两个独立但紧密协作的层次。

\textbf{问题二：缺乏全栈分层模型}
以使一门经典体系结构成为\emph{学科}的标准来衡量——稳定的硬件/软件接口契约（即ISA）、概率执行与确定性控制之间清晰的边界、以及定量的设计定律（Amdahl、Roofline）\cite{hennessy2017quantitative}——没有任何主流的模型原生工作能够同时提供这三者。
...
表~\ref{tab:artifact-comparison}进一步凝练了这一区分：在五项区分"学科"与"隐喻集合"的体系结构产物中，没有任何一项既有工作能同时覆盖超过一项，而ICA则将五项全部整合在一起。

\begin{table}[h]
\centering
\footnotesize
\caption{区分ICA与既有工作的体系结构产物。每一行列出一项产物、最接近的既有方法以及ICA的贡献；没有任何单一既有工作能同时具备全部五项。}
...
\toprule
\textbf{体系结构产物} & \textbf{最接近的既有方法} & \textbf{ICA（本文）} \\
\midrule
全栈分层模型 & Mi等人\cite{mi2025llmagentscomputersystems}：模块化映射，无层间契约 & 六层L1--L6，含接口契约 \\
双平面分离 & ArbiterOS\cite{arbiteros2025}：在"概率CPU"之上的治理层 & 双平面（概率执行 + 确定性控制） \\
接口契约 & 无；各层以非形式化方式描述 & 五个形式化的层间契约 \\
设计公理 & 无 & 六条移植而来的公理（局部性、抽象、概率执行等） \\
定量启发式 & 无 & 三条Amdahl式启发式 \\
\bottomrule
\end{tabularx}
\end{table}
