...
本文在第\ref{sec:icam}节提出的ICA模型将这些不同的"kernel"概念统一到一个6层体系结构中，并明确LLM应被定位为\textbf{概率执行平面}的核心（横跨物理执行层与推理服务层），而Agent OS是\textbf{确定性控制平面}中的编排层——两者属于不同平面，不应混为一谈。
...
这三种定位导致了完全不同的系统设计。
本文提出的ICA模型给出统一答案：LLM是概率执行平面的核心（类比CPU，横跨物理执行层与推理服务层），Agent OS是确定性控制平面的编排层（类比OS kernel），两者属于不同平面，不应混为一谈。
