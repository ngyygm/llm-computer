...
% TODO: Insert figure showing dual-plane architecture diagram (probabilistic execution plane vs. deterministic control plane)
\begin{figure}[t]
\centering
% \includegraphics[width=0.9\textwidth]{figures/dual_plane.pdf}
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2em}\textit{[Placeholder: Dual-plane architecture diagram showing the probabilistic execution plane (model forward pass, KV cache, attention, decoding) and the deterministic control plane (scheduler, permissions, state machine, audit log), with the control plane wrapping around and gating the execution plane.]}\vspace{2em}}}
\caption{The dual-plane architecture of model-native computing systems. The probabilistic execution plane handles understanding and generation; the deterministic control plane governs permissions, scheduling, and audit. The control plane mediates all side-effectful operations initiated by the execution plane.}
