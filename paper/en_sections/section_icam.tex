...
\paragraph{Classical source.}
The theoretical foundations of cache design\cite{drepper2007memory}.

\paragraph{ICAM embodiment.}
...
Every ICAM layer should employ locality-aware caching and prefetching. Law~I (Section~\ref{sec:law1}) quantifies this axiom into a computable formula.

\begin{axiom}[Layered Abstraction Axiom]
...
\paragraph{Classical source.}
ISA--microarchitecture separation; kernel--user-space separation\cite{hennessy2017quantitative,ostep_2023}.
...
\begin{axiom}[Probabilistic Execution Axiom]
...
\begin{axiom}[Virtualization Axiom]
...
\paragraph{Classical source.}
Virtual memory and process isolation\cite{ostep_2023}.
...
\begin{axiom}[Least Privilege Axiom]
...
\paragraph{Classical source.}
Operating-system security principles\cite{ostep_2023}.

\paragraph{ICAM embodiment.}
CaMeL's capability-security mechanism\cite{debenedetti2025camel}, which assigns fine-grained capability tokens to each agent operation; Codex's OS-level sandbox\cite{openai_codex_security_2026}, which restricts an agent's file-system and network access permissions.

\paragraph{Design implication.}
The L4--L5 interface must enforce per-operation permission checks via capability tokens (Section~\ref{sec:icam-interfaces}), and every L5 orchestration layer must support configurable permission policies that can be tightened or relaxed without modifying agent logic.

\begin{axiom}[Observability Axiom]
...
\paragraph{Classical source.}
System logging and performance monitoring\cite{linux_cgroup_2026}.

\paragraph{ICAM embodiment.}
The tracing mechanism in the OpenAI Agents SDK\cite{openai_codex_agentsdk_2026}, which logs each agent decision and tool invocation; the trace-grading methodology in agent-security research\cite{agenticsecurity2025}, which evaluates the safety of an agent's action trajectory.
