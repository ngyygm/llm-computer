...
\begin{tikzpicture}[
    ...
    rowhdr/.style={font=\small\bfseries, align=center, anchor=east},
...
% ---- Row headers ----
\node[rowhdr] at (-1.20, \yAc) {A1};
\node[rowhdr] at (-1.20, \yBc) {A2};
\node[rowhdr] at (-1.20, \yCc) {A3};
\node[rowhdr] at (-1.20, \yDc) {A4};
\node[rowhdr] at (-1.20, \yEc) {A5};
\node[rowhdr] at (-1.20, \yFc) {A6};
