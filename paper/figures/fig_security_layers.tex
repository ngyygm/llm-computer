...
\node[colhead, text=blue!60!black] at (\cx, \ystart + 0.45) {L5 Orchestration};
\pgfmathsetmacro{\cx}{\xstart + 5*(\cellw+\colsep) + \cellw/2}
\node[colhead, text=blue!60!black] at (\cx, \ystart + 0.45) {L6 Application};

% === column sub-headers: plane labels ===
\pgfmathsetmacro{\modelnativex}{\xstart + 0.5*(2*\cellw+\colsep)}
\node[font=\tiny\itshape, orange!70!black, anchor=north] at (\modelnativex, \ystart + 1.0) {Model-Native};
\pgfmathsetmacro{\classicalx}{\xstart + 3*\cellw + 2.5*\colsep + \cellw/2 - 0.15}
\node[font=\tiny\itshape, blue!60!black, anchor=north] at (\classicalx, \ystart + 1.0) {Classical};
...
\node[font=\scriptsize, anchor=west] at (\legx + 0.45, \legy + 0.175)
    {Primary impact};

% Secondary
\pgfmathsetmacro{\lx}{\legx + 2.4}
\draw[secondary] (\lx, \legy) rectangle +(0.35, 0.35);
\node[font=\scriptsize, anchor=west] at (\lx + 0.45, \legy + 0.175)
    {Secondary impact};

% Tertiary
\pgfmathsetmacro{\lx}{\legx + 5.2}
\draw[tertiary] (\lx, \legy) rectangle +(0.35, 0.35);
\node[font=\scriptsize, anchor=west] at (\lx + 0.45, \legy + 0.175)
    {Tertiary impact};

% Not applicable
\pgfmathsetmacro{\lx}{\legx + 8.0}
\draw[notapplic] (\lx, \legy) rectangle +(0.35, 0.35);
\node[font=\scriptsize, anchor=west] at (\lx + 0.45, \legy + 0.175)
    {Not applicable};

% Boundary legend entry
\pgfmathsetmacro{\lx}{\legx + 10.8}
\draw[very thick, dashed, black!55] (\lx, \legy + 0.175) -- ++(0.5, 0);
\node[font=\scriptsize, anchor=west] at (\lx + 0.6, \legy + 0.175)
    {Dual-plane boundary};
...
\pgfmathsetmacro{\bracetop}{\ystart + 1.3}
