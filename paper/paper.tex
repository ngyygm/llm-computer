...
\author{
林海\thanks{\texttt{ngyygm@outlook.com}\\ 清华大学深圳国际研究生院；鹏城实验室，深圳}
\quad 詹少雄\thanks{\texttt{zhansx24@mails.tsinghua.edu.cn}\\ 清华大学深圳国际研究生院}
\quad 郑海涛\thanks{\texttt{zheng.haitao@sz.tsinghua.edu.cn}\\ 清华大学深圳国际研究生院；鹏城实验室，深圳}
}
\date{2026年5月}
...
图~\ref{fig_analogy_map}给出了计算机体系结构与模型原生计算系统之间的类比映射总览。
\input{/home/linkco/exa/llm-computer/paper/figures/fig_analogy_map_cn.tex}
...
\input{/home/linkco/exa/llm-computer/paper/figures/fig_dual_stack_cn.tex}
...
\input{/home/linkco/exa/llm-computer/paper/figures/fig_coverage_map_cn.tex}
...
图~\ref{fig_dual_plane}展示了双平面架构中概率执行平面与确定性控制平面的分离。
\input{/home/linkco/exa/llm-computer/paper/figures/fig_dual_plane_cn.tex}
...
\item[ICAM体现] L2的KV缓存复用（相同前缀共享KV缓存）、L2的前缀缓存共享（多个请求共享公共system prompt的KV缓存）、L3的语义缓存（语义相似的查询复用之前的生成结果）。
\item[设计含义] ICAM的每一层都应基于局部性做缓存/预取。定律I（第~\ref{sec:laws}节）将这一公理量化为可计算的公式。
\end{description}
...
\item[ICAM体现] 模型升级（L2的实现变化）不应破坏Agent逻辑（L5的功能）；工具schema变更（L4的接口调整）不应导致记忆失效（L3的数据损坏）。
\end{description}
...
\item[ICAM体现] 整个ICAM模型的分层设计需要为概率性"留出空间"。L4-L5的接口契约不能要求工具调用的结果完全确定，而必须容忍语义层面的变化。L5的失败恢复机制不能依赖精确的输入回放，而需要语义级别的检查点。
\end{description}
...
\item[ICAM体现] MemGPT的虚拟上下文管理\cite{memgpt2023}；PagedAttention的页式KV管理\cite{kwon2023pagedattention}——将KV缓存按"页"组织，按需分配，回收不活跃页，与操作系统的虚拟内存管理如出一辙；Agent沙箱\cite{openai_codex_sandbox_2026}——将Agent的执行环境与宿主系统隔离，正如进程的地址空间隔离。
\end{description}
...
\item[ICAM体现] CaMeL的能力安全机制\cite{debenedetti2025camel}——为每个Agent操作分配细粒度的capability token；Codex的OS级沙箱\cite{openai_codex_security_2026}——限制Agent的文件系统和网络访问权限。
\end{description}
...
\item[ICAM体现] OpenAI Agents SDK的tracing机制\cite{openai_codex_agentsdk_2026}——记录Agent的每一步决策和工具调用；Agent安全研究中的trace grading\cite{agenticsecurity2025}——评估Agent操作轨迹的安全性。
\end{description}
...
\input{/home/linkco/exa/llm-computer/paper/figures/fig_law_curves_cn.tex}
...
\input{/home/linkco/exa/llm-computer/paper/figures/fig_agent_timeline_cn.tex}
...
图~\ref{fig_security_layers}展示了基于ICAM分层的安全防护架构。
\input{/home/linkco/exa/llm-computer/paper/figures/fig_security_layers_cn.tex}
...
\begin{longtable}{p{0.06\textwidth}p{0.06\textwidth}p{0.14\textwidth}p{0.28\textwidth}p{0.20\textwidth}p{0.16\textwidth}}
