...
At the quantitative level, we propose three Amdahl-style design heuristics---the \emph{Semantic Locality} heuristic, which relates KV-cache hit rates to inference speedup; the \emph{Context Budget} heuristic, which captures the dual constraint of window capacity and attention decay on effective working sets; and the \emph{Agent Speedup} heuristic, which characterizes the diminishing returns of multi-agent collaboration.
We are explicit that these are organizing back-of-envelope models rather than validated scaling laws: we illustrate their parameter ranges with published system-level data and identify systematic predictive validation as the principal open task.
Recent empirical evidence from the Anthropic 2026 agentic practices report further underscores an apparent tension: although multi-agent setups yield diminishing marginal returns, the aggregate economic impact is substantial, with organizations reporting hundreds of thousands of hours saved and significant acceleration of engineering delivery.
